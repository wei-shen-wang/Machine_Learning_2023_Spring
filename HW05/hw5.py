...
python_path = "~/.conda/envs/hw5/bin/python3.9"
data_dir_prefix = '/mnt/data/r112_whitebear/ML2023/HW5_data/DATA/'
data_dir = data_dir_prefix + 'rawdata'
    ...
    for s in ustring:
        ...
        for uchar in s:
            ...
            if inside_code == 12288:  # Full width space: direct conversion
                inside_code = 32
            elif (inside_code >= 65281 and inside_code <=
                  65374):  # Full width chars (except space) conversion
                inside_code -= 65248
...
def clean_corpus(prefix, l1, l2, ratio=9, max_len=1000, min_len=1):
    if Path(f'{prefix}.clean.{l1}').exists() and Path(
            f'{prefix}.clean.{l2}').exists():
...
and (prefix/f'valid.clean.{tgt_lang}').exists():
    ...
    for lang in [src_lang, tgt_lang]:
        train_f = open(
            os.path.join(data_dir, dataset_name, f'train.clean.{lang}'), 'w')
        valid_f = open(
            os.path.join(data_dir, dataset_name, f'valid.clean.{lang}'), 'w')
        count = 0
...
binpath = Path(data_dir_prefix + 'data-bin', dataset_name)
if binpath.exists():
    print(binpath, "exists, will not overwrite!")
else:
    os.system(
        f"{python_path} -m fairseq_cli.preprocess --source-lang {src_lang} --target-lang {tgt_lang} --trainpref {prefix}/train --validpref {prefix}/valid --testpref {prefix}/test --destdir {binpath} --joined-dictionary --workers 2"
    )

"""# Configuration for experiments"""

config = Namespace(
    datadir=data_dir_prefix + "data-bin/ted2020",
...
so that in the testing phase we can directly leverage fairseq's beam search decoder.
"""
...
class AttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.input_proj = nn.Linear(input_embed_dim,
                                    source_embed_dim,
                                    bias=bias)
        self.output_proj = nn.Linear(input_embed_dim + source_embed_dim,
                                     output_embed_dim,
                                     bias=bias)
    ...
    def forward(self, inputs, encoder_outputs, encoder_padding_mask):
        ...
        if encoder_padding_mask is not None:
            ...
            attn_scores = (attn_scores.float().masked_fill_(
                encoder_padding_mask, float("-inf")).type_as(attn_scores)
...
For strong baseline, 
please refer to the hyperparameters for *transformer-base* in Table 3 in [Attention is all you need](#vaswani2017)
"""

arch_args = Namespace(encoder_embed_dim=512,
...
* sometimes the ground truth may not be the only answer. 
* Thus, when calculating loss, we reserve some probability for incorrect labels
* avoids overfitting
...
class NoamOpt:
    ...
    def rate(self, step=None):
        ...
        if step is None:
            step = self._step
        return 0 if not step else self.factor * get_rate(
            self.model_size, step, self.warmup)
...
def train_one_epoch(epoch_itr,
    ...
    itr = iterators.GroupedIterator(
        itr,
        accum_steps)  # gradient accumulation: update every accum_steps samples
    ...
    for samples in progress:
        ...
        optimizer.multiply_grads(
            1 /
            (sample_size or
             1.0))  # (sample_size or 1.0) handles the case of a zero gradient
...
def inference_step(sample, model):
    ...
    for i in range(len(gen_out)):
        ...
        hyps.append(
            decode(
                gen_out[i][0]
                ["tokens"],  # 0 indicates using the top hypothesis in beam
...
epoch_itr = load_data_iterator(task, "train", config.start_epoch,
                               config.max_tokens, config.num_workers)
try_load_checkpoint(model, optimizer, name=config.resume)
while epoch_itr.next_epoch_idx <= config.max_epoch:
    # train for one epoch
    train_one_epoch(epoch_itr, model, task, criterion, optimizer,
                    config.accum_steps)
    stats = validate_and_save(model,
                              task,
                              criterion,
                              optimizer,
...
os.system(
    f"{python_path} ~/fairseq/scripts/average_checkpoints.py --inputs {checkdir} --num-epoch-checkpoints 5 --output {checkdir}/avg_last_5_checkpoint.pt"
)
...
def generate_prediction(model, task, split="test", outfile="./prediction.txt"):
    ...
    itr = load_data_iterator(task, split, 1, config.max_tokens,
                             config.num_workers).next_epoch_itr(shuffle=False)
